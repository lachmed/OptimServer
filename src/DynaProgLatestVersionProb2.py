...
def calculerFonctionObjective(S, J, j, res, data):
    if S == 1:
    ...
    if res[tuple(J[:S-1])][0] + data[j][0] > data[j][1]:
        T = (res[tuple(J[:S-1])][0] + data[j][0]) - data[j][1]
        E = 0
        ...
        T = 0
        E = data[j][1] - (res[tuple(J[:S-1])][0] + data[j][0])
...
def F(S, J, res, data):
    ...
    if S == 1:
        res[tuple(J)] = [data[J[0]][0], calculerFonctionObjective(
            S, J, J[0], res, data), J, J[0]]
        return
...
# si la taille est deux on teste les deux cas possibles et on prend leur min
    if S == 2:
        ...
        case2 = J.copy()
        tmp.append(calculerFonctionObjective(
            S, case1, J[0], res, data) + res[tuple(case1[:S-1])][1])
        tmp.append(calculerFonctionObjective(
            S, case2, J[1], res, data) + res[tuple(case2[:S-1])][1])
...
# et prendre leurs min
    ...
    for i in range(S):
        ...
        if finnished > data[J[i]][1]:
            t = data[J[i]][3] * (finnished - data[J[i]][1])
        elif finnished == data[J[i]][1]:
            t = 0
        else:
            t = data[J[i]][2] * (data[J[i]][1] - finnished)
...
def dynamiqueP2(instance):
    ...
    for r in range(1, len(J)+1):
        if cpt < 2:
            ...
            cpt += 1
            for subSet in state:
                F(len(subSet), list(subSet), res, data)
        else:
...
# renverser pour retrouver l'ordre début vers fin
    ...
    ress.append(time.time()-start)

    data2 = [data[sequance[0]][0]]

    for i in range(1, len(sequance)):
        data2.append(data2[i-1] + data[sequance[i]][0])
